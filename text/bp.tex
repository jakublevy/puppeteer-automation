...
\makeatother
	...
	\newpage
	\section{Úvod}
	%TODO: napsat obecny uvod o automatizaci prohlizecu
	Dovolíme si ještě upozornit čtenáře -- na konci této práce se nachází \hyperref[glossary]{Slovníček vybraných pojmů}, v~němž jsou uvedeny definice vybraných pojmů.
	...
	Přikládáme ještě tabulku primárně určenou pro rychlou orientaci v~této sekci a~jednoduché porovnání.
{
	...
	%selenium IDE, Katalon Recorder, Ranorex Recorder, Headless Recorder
...
Object.assign(String.prototype, {
	...
	\paragraph{Vybrané metody}
	\begin{methods}
	\lstinline[style=JSProgrammersDocs]|function transformLocators(locators)| vyextrahuje CSS lokátor z~pole\linebreak\lstinline|locators|, vrátí dvě hodnoty: pole zbylých lokátorů a~CSS lokátor.
	\end{methods}
	...
	\begin{methods}
		...
		\lstinline[style=JSProgrammersDocs]|async launch(options)| připojí Puppeteer na prohlížeč voláním\linebreak \lstinline|puppeteer.launch(options)|. Následně přidá do objektu prohlížeče rozšíření z~\ref{docs:browserExtensions}.
	...
	\begin{methods}
		...
		\lstinline|_addPermissionToCurrPage(permission)| přidá požadavek na oprávnění\linebreak\lstinline|permission| pro aktuální URL aktivní stránky.
		...
		\lstinline|_genWaitForIdentifier(id)| vygeneruje volání \lstinline|Page.waitForXPath| nebo\linebreak\lstinline|Page.waitForSelector| dle typu identifikátoru \lstinline|id|.
		...
		\lstinline|_plainCode(codeBlocks)| převede bloky kódu vygenerované metodou\linebreak\lstinline|_codeGenBlock| do kódu pro Puppeteer.
		
		\enlargethispage{\baselineskip}
		Mnoho dalších metod generujících volání: \lstinline|_browserFuncCall|,\linebreak \lstinline|_genExecRawCodeInsidePage|, \lstinline|_genIdentifierFuncCall|, \lstinline|_pageFuncCall|, ... Více informací o~těchto a~dalších metodách je uvedeno v~komentářích ve zdrojovém\linebreak souboru.
	\end{methods}
...
%	\paragraph{Formulář}
	...
	\begin{methods}
		\enlargethispage{\baselineskip}
		\lstinline[style=MyCSharpDocs]|private void MainForm_Load(object sender, EventArgs e)| spuštěna při\linebreak načtení formuláře, načte uložené náhledy nahrávek a~zkontroluje existenci interpreteru Node.js.
		
		\lstinline[style=MyCSharpDocs]|protected override bool ProcessCmdKey(ref Message msg, Keys keyData)|
		
		\lstinline[style=MyCSharpDocs]|public void SwitchToEditMode(Thumbnail t)| přetížení definující klávesovou zkratku Shift+Delete při editaci nahrávky. Tato zkratka odstraní akci s~vlastností \lstinline|Focus = true|.
		
		Další metody zpracovávající události UI: \lstinline|addNewRecordingButton_Click|,\linebreak\lstinline|MainForm_FormClosing|, \lstinline|recorderToolStripMenuItem_Click|, ...
		
		A~metody upravující UI: \lstinline|LoadThumbnailsUi|, \lstinline|NodeInterpterNotWorking|,\linebreak\lstinline|PerformSettingsChecks|, \lstinline|SetEditUiVisiblity|, \lstinline|SwitchToEditMode|, \lstinline|UiChange| ...
...
%	\paragraph{Formulář}
	...
	\begin{methods}
		...
		Metody obsahující logiku prohazování pořadí akcí\footnote{Tyto metody jsou využity při stisku tlačítek \includegraphics{upArrow.png} a~\includegraphics{downArrow.png} nacházejících se na \cref{fig:actionUserControl}.}.: \lstinline|downButton_Click|,\linebreak\lstinline|GetNextVisiblePosition|, \lstinline|GetPreviousVisiblePosition|, \lstinline|UpdateUpDownButtons|, \lstinline|upButton_Click|.
		
		A~metody zpracovávající další události UI: \lstinline|deleteButton_Click|,\linebreak\lstinline|jsonEditButton_Click|, \lstinline|selectorTextBox_TextChanged|,\newline \lstinline|typeComboBox_SelectedValueChanged|, \lstinline|valueTextBox_TextChanged|, ...
	...
	\paragraph{Vybrané metody}
	\begin{methods}
		\lstinline[style=MyCSharpDocs]|private void AddErrorToReplayViewForm(string msg, int id)|
		
		\lstinline[style=MyCSharpDocs]|public void BindEdit(CurrentEdit ce)|
		
		\lstinline[style=MyCSharpDocs]|private void FinishReplay()|
		
		\lstinline[style=MyCSharpDocs]|private List<Recording> GetAllActions()|
		
		\lstinline[style=MyCSharpDocs]|private List<string> GetEventsToRecord()|
		
		\lstinline[style=MyCSharpDocs]|private List<Recording> GetRecordingsForOptimize()|
		
		\lstinline[style=MyCSharpDocs]|private void LoadAction(dynamic action)|
		
		\lstinline[style=MyCSharpDocs]|private void LoadRecording(Recording r)|
		
		\lstinline[style=MyCSharpDocs]|private void RecordingTask()|
		
		\lstinline[style=MyCSharpDocs]|private void ReplayTask()|
		
		\lstinline[style=MyCSharpDocs]|private HashSet<string> ScrapeWebsitesFromRecordings(List<Recording> recordings)|
		
		
		\lstinline[style=MyCSharpDocs]|private void StartNodeJsProcess()|
		
		
		%TODO: dalsi udalosti plus funkce menici ui
	\end{methods}
	...
	\paragraph{Vybrané metody}
	\begin{methods}
		\lstinline[style=MyCSharpDocs]|public void BindThumbnail(Thumbnail t)| nastaví UI komponenty podle\linebreak parametru \lstinline|t|.
	\end{methods}
	...
	\begin{methods}
		...
		\lstinline[style=MyCSharpDocs]|public static void SavePuppeteerConfiguration(PuppeteerOptions po)| uloží nastavení uživatelské komponenty \ref{docs:puppeteerOptionsUserControl}.
	...
	Reprezentace nahrané akce s~nastavením její uživatelské komponenty\linebreak\texttt{ActionUserControl} (\ref{docs:auc}).
	\paragraph{Třídy}
	\begin{methods}
		\texttt{Recording}
