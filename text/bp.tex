	...
	\paragraph{Další pokročilé funkce}
{
	...
	Celá frontendová část je projektem Visual Studia 2019, ve kterém taktéž probíhal její vývoj. Vzhledem k~tomu, že využíváme WinForms, bylo Visual Studio jasnou volbou díky existenci designeru.
	...
	\begin{enumerate}
		\item Stáhneme přílohu práce\footnote{Příloha je dostupná i~na GitHubu~\cite{projectGithub}, commit ID ... odpovídá verzi, která byla odevzdána s~tímto textem.}
		\item Nainstalujeme Node.js\footnote{Mohli bychom distribuovat interpreter přímo se zdrojovým kódem, nicméně nepříjde nám jako dobrý nápad zmrazovat závislost o~velikosti \textasciitilde~68~MB. Lepší varianta podle nás je, když všechny aplikace vyžadující Node.js používají jeden systémový.} 15.5.0 (dle~\cref{tab:usedSoftware}) odpovídající architektuře OS
		\begin{itemize}[leftmargin=0pt,label={}, align=left,topsep=0pt]
			\item 64bit instalátor: \url{https://nodejs.org/download/release/v15.5.0/node-v15.5.0-x64.msi}
			\item 32bit instalátor: \url{https://nodejs.org/download/release/v15.5.0/node-v15.5.0-x86.msi}
		\end{itemize}
	\end{enumerate}
