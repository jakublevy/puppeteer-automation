...
async function receiveMsg(sock) {
	...
	Textový editor zobrazující vygenerovaný kód pro Puppeteer. Použitý textový editor je komponenta \texttt{ScintillaNET.Scintilla} projektu \texttt{ScintillaNET} \cite{scintillaGithub} dostupného pod MIT licencí v~souladu s~kterou je komponenta využita. 
	\paragraph{Formuláře}
	\begin{figure}[H]
...
public void SetEditorText(string txt) { textEditor.Text = txt; }
	\end{lstlisting}
