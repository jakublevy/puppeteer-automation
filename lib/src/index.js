...
const laptopViewport = { width: 1366, height: 768 }
...
(async () => {
  ...
  //  await recorder.connect({ browserURL: 'http://localhost:9222', defaultViewport: laptopViewport })
...
rl.on('line', async (line) => {
    if(line === 's')
        ...
        console.log(cg.codeGen(recorder.optimizedActions))
    }
    else if(line === 'r') {
        const cg = new CodeGenerator({
            puppeteerOptions: { headless: false, slowMo: 100, defaultViewport: null},
