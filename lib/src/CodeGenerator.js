...
class CodeGenerator {
    ...
    codeGenById(actions, id) {
        const idx = actions.findIndex(a => a.id === id)
        return this.codeGenByIdx(actions, idx)
    }
    ...
    _processStartupHits(actions) {
        if (!actions.empty() && actions[0].type === 'startupHints') {
         ...
         //   this._idxOfDeclaredPages = new Set(Array(actions[0].existingPagesMaxIdx))
    ...
    _codeGenBlock(actions, i) {
        ...
        switch (actions[i].type) {
          ...
          //      this._idxOfDeclaredPages.delete(actions[i].idx)
                break
        ...
        if (main !== undefined) {
            ...
            if(i + 1 < actions.length && actions[i+1].type === 'pageUrlChanged' && actions[i+1].implicitNav) {
                ...
                //     main = `${this._removeTrailingCatch(prologue[0])}.then(${this._removeLeadingAwait(main)})`
                //     main = this._catchAndLogErrors(main)
                //     prologue = []
                // }
            }
    ...
    _wrapWithPromiseAll(toWrap) {
        //TODO: remove trailing catch if exists for each in array
    ...
    _plainCode(codeBlocks) {
        ...
        if(startup !== '') {
         ...
         //   code += (Array.from(Array(c.existingPagesMaxIdx + 1).keys()).map((idx) => `${this._makeIndent()}let page${idx} = pages[${idx}]`)).join('\n')
            code += '\n'
    ...
    _declaration(act) {
        ...
        //this._idxOfDeclaredPages.add(act.idx)
        return decl
    }
