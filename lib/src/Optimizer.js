    ...
    optimize(acts) {
        ...
        for(let i = acts.length - 1; i >= 0; --i) {
            if(i - 1 >= 0) {
                ...
                else if(acts[i].type === 'change' && acts[i - 1].type === 'click' && this._theSameElement(acts[i-1], acts[i])) {
                    acts.removeAt(i-1)
                    continue
                }

                else if(acts[i-1].type === 'paste' && acts[i].type === 'change') {
                    acts.removeAt(i)
                    continue
                }
